...
function initializeBot() {
	...
	$("#msgTxt").focus();
	$("#chat friend").show();
	//sendMessage('greetemit','Greetings! Iam Vaani. How may I help you?',false);
}

socket.on('receive', function (msg) {
    console.log("OKOKOK" , msg)
	receiveMessage(msg)
});
...
function sendMessage(type, msg, isHuman) {

console.log("socket" , socket)
	console.log('sendMessage::type=', type, ', msg=', msg, 'isHuman=', isHuman)
	socket.emit(type, {
		data: msg,
		isHuman: isHuman
	})
...
function createNewMessage(message) {
	...
	$chatlogs.append(
		$('<div/>', {'class': 'chat friend'}).append(
			$('<div/>', {'class': 'user-photo'}).append($('<img src="../static/images/bot-new.GIF" />')), 
			$('<p/>', {'class': 'chat-message', 'text': message})));
			
	if(message.indexOf("some quick links")>0){
		$chatlogs.append("<div class='chatbot'><ul class='quick__linkslist'><li class='quick--links'><span title='Ramayan' onclick='javascript:autosubmit('Ramayan')'>Ramayan</span></li><li class='quick--links'><span title='MahaBharat' onclick='javascript:autosubmit('MahaBharat')'>MahaBharat</span></li></ul></div>");
	}
